...
import { AlertTriangle, Check, ChevronDown, ChevronUp, Crown, Loader2, PlusCircle, Save, Sparkles, Trash2 } from 'lucide-react';
...
function convertSharedCard(card: any, subject: string, level: EducationLevel): Flashcard {
  return {
    ...
    difficulty: (card.difficulty || 'medium') as Flashcard['difficulty'],
    ...
    cardContentType: card.cardContentType,
    educationLevel: (card.educationLevel || level) as EducationLevel,
  };
}
  ...
  return (
    <article className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
      <div className="flex items-start gap-3">
        <span className="shrink-0 rounded-lg bg-orange-50 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/20 px-2 py-1 text-[11px] font-bold text-orange-700 dark:text-orange-300">#{index + 1}</span>
        <div className="min-w-0 flex-1">
          {card.topic && <p className="mb-1 truncate text-[10px] font-bold uppercase tracking-wider text-slate-400">{card.topic}</p>}
          <p className="text-sm font-semibold leading-snug text-slate-900 dark:text-white">{card.front}</p>
        </div>
        <button type="button" onClick={onRemove} aria-label="Remover card" className="p-1 text-slate-400 hover:text-rose-500"><Trash2 className="h-4 w-4" /></button>
      </div>
      <div className="mt-3 space-y-2 border-t border-slate-100 dark:border-slate-700 pt-3">
        <p className="whitespace-pre-line text-xs text-slate-700 dark:text-slate-200"><strong className="text-emerald-600 dark:text-emerald-400">Resposta:</strong> {card.back}</p>
        {card.explanation && <p className="whitespace-pre-line text-xs text-slate-600 dark:text-slate-300"><strong className="text-amber-600 dark:text-amber-400">Explicação:</strong> {card.explanation}</p>}
      </div>
    </article>
  );
}
...
export const StudioView: React.FC<StudioViewProps> = ({ decks, stats, onSaveNewDeck, onOpenSubscription, initialDeck, onConsumedInitialDeck }) => {
  ...
  useEffect(() => {
    if (subject.trim().length < 2) {
      setDetectedLevels([]); setLevelCurricula(new Map()); setSelectedSubtopics(new Set()); setExpanded(new Set()); setSubjectSuggestions([]); return;
    }
    const query = upper(subject.trim());
    const local = Array.from(new Set([...decks.flatMap(deck => [deck.category, deck.title, ...deck.cards.map(card => card.subject || '')]), ...getCuratedSubjectSuggestions(query)]))
      .filter(Boolean).map(upper).filter(item => normalize(item).includes(normalize(query))).slice(0, 8);
    setSubjectSuggestions(local);
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        await loadAllLevelCurricula(query, levels.length ? levels : [{ level: primary, label: primary, icon: '🎓', reason: '', priority: 1 } as LevelInfo], updated => {
          if (!cancelled) setLevelCurricula(new Map(updated));
        });
      } catch (err: any) {
        if (!cancelled) setError(err?.message || 'Não foi possível consultar a grade no banco.');
      }
    }, 350);
  ...
  useEffect(() => {
    ...
    const all = new Set(allSubtopics);
    setSelectedSubtopics(all);
    setExpanded(new Set(curriculum.map(category => category.category)));
  }, [curriculum.length, educationLevel]);

  useEffect(() => {
    if (!subject.trim() || !selectedSubtopics.size) { setBankReady(0); setBankMissing(0); return; }
    let cancelled = false;
    const timer = window.setTimeout(async () => {
      const availability = await queryBankAvailability(subject.trim(), Array.from(selectedSubtopics), educationLevel);
      if (!cancelled) { setBankReady(availability.totalReadyCards); setBankMissing(availability.needsGeneration.length); }
    }, 200);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [subject, selectedSubtopics, educationLevel]);
  ...
  const toggleCategory = (category: CurriculumCategory) => {
    ...
    setSelectedSubtopics(current => {
      ...
      values.forEach(value => all ? next.delete(value) : next.add(value));
      return next;
    });
  };
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const shared = await fetchSharedCards(normalizedSubject, selected, educationLevel, target);
      ...
      if (!cards.length) throw new Error(bankReady > 0 ? 'Os cards disponíveis já estão no seu baralho. Não gerei conteúdo duplicado.' : 'Não foi possível obter cards do banco nem da IA. Tente novamente.');
  ...
  const handleSaveGenerated = () => {
    ...
    const deck: Deck = existing
      ...
      : { id: `deck-${Date.now()}`, title, category: upper(subject.trim()) || 'GERAL', description: '', color: '#60a5fa', accentBorder: 'border-l-primary', cards: generatedCards, createdAt: new Date().toISOString() };
    onSaveNewDeck(deck);
    const count = generatedCards.length;
    setGeneratedCards([]);
    setMessage(`${count} cards salvos no baralho ${title}.`);
    if (initialDeck) onConsumedInitialDeck?.();
  };

  const handleManualCard = (card: Flashcard, targetDeckName: string) => {
    const title = upper(targetDeckName.trim());
    const existing = decks.find(deck => normalize(deck.title) === normalize(title));
    if (existing) onSaveNewDeck({ ...existing, cards: [card, ...existing.cards] });
    else onSaveNewDeck({ id: `deck-${Date.now()}`, title, category: upper(card.subject || 'GERAL'), description: '', color: '#60a5fa', accentBorder: 'border-l-primary', cards: [card], createdAt: new Date().toISOString() });
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 pb-28 animate-fade-in">
      <div className="flex justify-center">
        <div className="flex gap-1 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <button type="button" onClick={() => setMode('ia')} className={`rounded-xl px-6 py-3 text-xs font-extrabold flex items-center gap-2 ${mode === 'ia' ? 'bg-[#6658f5] text-white' : 'text-slate-500 dark:text-slate-400'}`}><Sparkles className="h-4 w-4" /> GERAR COM IA</button>
          <button type="button" onClick={() => setMode('manual')} className={`rounded-xl px-6 py-3 text-xs font-extrabold flex items-center gap-2 ${mode === 'manual' ? 'bg-[#6658f5] text-white' : 'text-slate-500 dark:text-slate-400'}`}><PlusCircle className="h-4 w-4" /> MANUAL</button>
        </div>
      </div>

      {mode === 'ia' ? (
        <section className="space-y-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-7">
          <div><p className="text-[11px] font-black uppercase tracking-wider text-[#6658f5]">GERAR COM IA</p><h1 className="mt-1 text-2xl font-black text-slate-900 dark:text-white">Monte sua grade e receba os cards certos.</h1></div>

          <div className="relative">
            <label className="mb-2 block text-xs font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">MATÉRIA / ASSUNTO *</label>
            ...
            {showSuggestions && subjectSuggestions.length > 0 && <div className="absolute z-30 mt-2 w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">{subjectSuggestions.map(item => <button key={item} type="button" onMouseDown={() => applySuggestion(item)} className="block w-full px-4 py-3 text-left text-xs font-bold uppercase text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800">{item}</button>)}</div>}
          </div>

          <div>
            <div className="mb-2 flex items-center justify-between"><label className="text-xs font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">NÍVEL</label></div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{EDUCATION_LEVEL_META.map(level => {
              const active = educationLevel === level.value;
              const detected = detectedLevels.length === 0 || detectedLevels.some(item => item.level === level.value);
              return <button key={level.value} type="button" disabled={!detected} onClick={() => { setEducationLevel(level.value); setEducationLevelLocked(true); setSelectedSubtopics(new Set()); }} className={`rounded-xl border px-3 py-3 text-xs font-extrabold transition ${active ? 'border-[#6658f5] bg-[#6658f5] text-white' : 'border-slate-200 bg-white text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200'} ${!detected ? 'cursor-not-allowed opacity-35' : ''}`}>{level.icon} {upper(level.label)}</button>;
            })}</div>
          </div>

          {curriculum.length > 0 && <div className="space-y-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
            <div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-black uppercase text-slate-900 dark:text-white">GRADE CURRICULAR</h2><p className="mt-1 text-[11px] text-slate-500">{selectedSubtopics.size} de {allSubtopics.length} tópicos/subtópicos selecionados</p></div><button type="button" onClick={toggleAll} className="rounded-xl bg-[#6658f5]/10 px-3 py-2 text-[11px] font-black text-[#5143d9] dark:text-[#a9a0ff]">{allSelected ? 'DESMARCAR TODOS' : 'SELECIONAR TODOS'}</button></div>
            <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700">{curriculum.map(category => {
              const items = category.topics || [];
              const selectedCount = items.filter(item => selectedSubtopics.has(item)).length;
              const open = expanded.has(category.category);
              return <div key={category.category} className="border-b border-slate-100 last:border-0 dark:border-slate-800"><div className="flex items-center gap-2 bg-slate-50 px-3 py-3 dark:bg-slate-950"><input type="checkbox" checked={selectedCount === items.length && items.length > 0} ref={el => { if (el) el.indeterminate = selectedCount > 0 && selectedCount < items.length; }} onChange={() => toggleCategory(category)} className="h-4 w-4 accent-[#6658f5]" /><button type="button" onClick={() => setExpanded(current => { const next = new Set(current); next.has(category.category) ? next.delete(category.category) : next.add(category.category); return next; })} className="flex-1 text-left text-xs font-black uppercase text-slate-800 dark:text-slate-100">{category.category}<span className="ml-2 text-[10px] font-normal text-slate-500">{selectedCount}/{items.length}</span></button>{open ? <ChevronUp className="h-4 w-4 text-slate-400" /> : <ChevronDown className="h-4 w-4 text-slate-400" />}</div>{open && <div className="grid gap-1 p-3 sm:grid-cols-2">{items.map(item => <label key={item} className="flex cursor-pointer items-start gap-2 rounded-xl px-2 py-2 hover:bg-slate-50 dark:hover:bg-slate-800"><input type="checkbox" checked={selectedSubtopics.has(item)} onChange={() => setSelectedSubtopics(current => { const next = new Set(current); next.has(item) ? next.delete(item) : next.add(item); return next; })} className="mt-0.5 h-4 w-4 accent-[#6658f5]" /><span className="text-xs text-slate-700 dark:text-slate-200">{upper(item)}</span></label>)}</div>}</div>;
            })}</div>
          </div>}

          {subject && selectedSubtopics.size > 0 && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-[11px] text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-200"><b>CONTEÚDO DO BANCO:</b> {bankReady} CARDS PRONTOS. <b>FALTANDO:</b> {bankMissing} TÓPICOS. O BANCO É SEMPRE CONSULTADO ANTES DA IA.</div>}
          ...
          <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/30"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600 dark:text-amber-400" /><p className="text-[11px] leading-relaxed text-amber-800 dark:text-amber-200">A IA pode cometer erros. O MemoriaFlash usa primeiro o conteúdo validado do banco compartilhado e só gera com IA o que ainda estiver faltando.</p></div>
          {error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs text-rose-700 dark:border-rose-800 dark:bg-rose-950/30 dark:text-rose-300">{error}</div>}
          {message && <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300">{message}</div>}
