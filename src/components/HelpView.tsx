...
export const HelpView: React.FC<{ currentLanguage?: string }> = () => {
  ...
  const submitFeedback = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // O servidor sempre responde com status 200, mesmo em erro interno —
      // ele sinaliza falha real via `{ ok: false }` no corpo da resposta.
      ...
      // CORREÇÃO: antes mostrava "enviado" mesmo quando o envio falhava de
      // verdade (rede offline, servidor fora do ar) — o feedback do usuário
      // era perdido silenciosamente e ele nunca ficava sabendo. Agora
      // mostramos um erro real e mantemos o texto digitado para reenvio.
      setSendError(true);
    } finally {
      setSending(false);
    }
      ...
      <div className="glass-card rounded-2xl p-5 border border-[#adc6ff]/30 space-y-4">
        ...
        {sent ? (
          ...
          <form onSubmit={submitFeedback} className="space-y-3">
            {sendError && (
              <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-[11px] text-rose-300">
                <span>⚠️ Não foi possível enviar. Verifique sua conexão e tente novamente — seu texto foi mantido.</span>
              </div>
            )}
