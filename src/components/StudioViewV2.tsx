  ...
  return (
    <article className="bg-[#0b1a2a]/95 border border-[#adc6ff]/20 rounded-2xl p-4 shadow-xl">
      <div className="flex items-start gap-3">
        ...
        <button type="button" onClick={onRemove} className="p-1 text-slate-500 hover:text-red-400" title="Remover card"><Trash2 className="w-4 h-4" /></button>
      </div>
...
export const StudioView: React.FC<StudioViewProps> = ({ decks, stats, onSaveNewDeck, onOpenSubscription, initialDeck, onConsumedInitialDeck }) => {
  ...
  const existingFronts = useMemo(() => Array.from(new Set(decks.flatMap((deck) => deck.cards.map((card) => normalizeFront(card.front))).filter(Boolean))), [decks]);

  const existingTopics = useMemo(() => {
    const selected = normalizeTopic(subject);
    if (!selected) return [];
    const values = decks
      .filter((deck) => {
        const candidates = [deck.category, deck.title, ...deck.cards.map(card => card.subject || '')].filter(Boolean).map(normalizeTopic);
        return candidates.some(value => value === selected || value.includes(selected) || selected.includes(value));
      })
      .flatMap(deck => deck.cards.map(card => card.topic || '').filter(Boolean));
    const unique = new Map<string, string>();
    values.forEach(value => { const key = normalizeTopic(value); if (key && !unique.has(key)) unique.set(key, value.trim()); });
  ...
  const existingTopicKeys = useMemo(() => new Set(existingTopics.map(normalizeTopic)), [existingTopics]);
  ...
  const handleGenerate = async () => {
    ...
    if (!canGenerateAICards(stats, cardCount)) return setError(`Você já utilizou ${generated}/${FREE_AI_CARD_LIMIT} cards gratuitos. Assine o PRO para gerar cards ilimitados.`);
    setLoading(true);
    try {
      const cards = await generateAICards(subject.trim(), requestedTopics, cardCount, 'medio', existingFronts);
      setGeneratedCards(cards); setMessage(`${cards.length} cards gerados com sucesso.`);
  ...
  return (
    <div className="max-w-3xl mx-auto pb-24 animate-fade-in space-y-5">
      <div className="flex justify-center"><div className="bg-[#0b1a2a] p-1.5 rounded-2xl border border-[#424754]/40 flex gap-2">
        <button type="button" onClick={() => setMode('ia')} className={`px-6 py-2.5 rounded-xl font-extrabold text-xs flex items-center gap-2 ${mode === 'ia' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}><Sparkles className="w-4 h-4" /> Gerar com IA</button>
        <button type="button" onClick={() => setMode('manual')} className={`px-6 py-2.5 rounded-xl font-extrabold text-xs flex items-center gap-2 ${mode === 'manual' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}><PlusCircle className="w-4 h-4" /> Criar manualmente</button>
      </div></div>

      {mode === 'ia' ? (
        <section className="bg-[#0b1a2a]/90 border border-[#adc6ff]/20 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5">
          <div><h1 className="text-xl font-extrabold text-white">Gerar flashcards</h1><p className="text-xs text-slate-400 mt-1">Crie cards automaticamente para estudar no MemoriaFlash.</p></div>

          {!stats.isPro && <div className="rounded-2xl border border-blue-500/30 bg-blue-500/10 p-4">
            <div className="flex items-center justify-between gap-3 mb-2"><div><p className="text-xs font-extrabold text-blue-200">Limite gratuito</p><p className="text-[11px] text-slate-400">{generated} de {FREE_AI_CARD_LIMIT} cards gerados</p></div><span className="text-sm font-black text-blue-300">{remaining} restantes</span></div>
            <div className="h-2 rounded-full bg-slate-800 overflow-hidden"><div className="h-full bg-blue-500 transition-all" style={{ width: `${Math.min(100, (generated / FREE_AI_CARD_LIMIT) * 100)}%` }} /></div>
            {remaining === 0 && <button type="button" onClick={onOpenSubscription} className="mt-3 w-full py-2.5 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-300 font-bold text-xs flex items-center justify-center gap-2"><Crown className="w-4 h-4" /> Assinar PRO — geração ilimitada</button>}
          </div>}
          ...
          <div className="space-y-4 text-left">
            ...
            {existingTopics.length > 0 && <div className="rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-3 space-y-2">
              <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-400" /><p className="text-[11px] font-extrabold text-emerald-300">Tópicos já existentes nesta matéria</p></div>
              <div className="flex flex-wrap gap-1.5">{existingTopics.map(topic => <span key={normalizeTopic(topic)} className="px-2.5 py-1.5 rounded-lg bg-emerald-500/15 border border-emerald-400/30 text-[10px] font-bold text-emerald-200">✓ {topic}</span>)}</div>
              <p className="text-[10px] text-slate-500">Os tópicos em verde já possuem cards. Você pode gerar novos cards para eles ou escolher outros tópicos.</p>
            </div>}
            ...
            {requestedTopics.length > 0 && <div className="flex flex-wrap gap-1.5">{requestedTopics.map(topic => { const exists = existingTopicKeys.has(normalizeTopic(topic)); return <span key={`${normalizeTopic(topic)}-${topic}`} className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border ${exists ? 'bg-emerald-500/15 border-emerald-400/30 text-emerald-200' : 'bg-blue-500/10 border-blue-400/20 text-blue-200'}`}>{exists ? '✓ Já existe: ' : '+ Novo: '}{topic}</span>; })}</div>}

            <div><label className="block text-xs font-bold uppercase tracking-wider text-[#adc6ff] mb-2">Quantidade</label>
            ...
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 flex items-start gap-3" role="alert"><AlertTriangle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" /><div><p className="text-xs font-extrabold text-amber-200">Atenção ao conteúdo gerado pela IA</p><p className="text-[11px] leading-relaxed text-amber-100/80 mt-1">O gerador de flash cards pode cometer erros. Por isso, lembre-se de conferir informações geradas.</p></div></div>
        ...
        <section className="bg-[#0b1a2a]/90 border border-[#adc6ff]/20 rounded-3xl p-6 sm:p-8 shadow-2xl"><h1 className="text-xl font-extrabold text-white mb-1">Criar card manualmente</h1><p className="text-xs text-slate-400 mb-5">Cards criados aqui pertencem aos seus baralhos e não são publicados para outros usuários.</p><ManualCardForm existingDecks={decks.map((deck) => deck.title)} subjects={Array.from(new Set(decks.flatMap((deck) => [deck.category, ...deck.cards.map((card) => card.subject).filter((s): s is string => !!s)])))} onAddCardDirectly={handleManualCard} /></section>
      )}

      {generatedCards.length > 0 && mode === 'ia' && <section className="space-y-3"><div className="flex items-center justify-between px-2"><h2 className="text-base font-extrabold text-[#adc6ff]">Cards gerados ({generatedCards.length})</h2><button type="button" onClick={() => setGeneratedCards([])} className="text-xs text-slate-500 hover:text-rose-400">Descartar</button></div>{generatedCards.map((card, index) => <GeneratedCard key={card.id} card={card} index={index} onRemove={() => setGeneratedCards((prev) => prev.filter((c) => c.id !== card.id))} />)}<button type="button" onClick={handleSaveGenerated} className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-extrabold flex items-center justify-center gap-2"><Save className="w-5 h-5" /> Salvar no Baralho</button></section>}
    </div>
  );
};
