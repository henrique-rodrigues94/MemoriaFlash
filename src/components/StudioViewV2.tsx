...
export const StudioView: React.FC<StudioViewProps> = ({ decks, stats, onSaveNewDeck, onOpenSubscription, initialDeck, onConsumedInitialDeck }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (!canGenerateAICards(stats, cardCount)) {
      return setError(`Você já utilizou ${generated}/${FREE_AI_CARD_LIMIT} cards gratuitos. Assine o PRO para gerar cards ilimitados.`);
    }
    setLoading(true);
    try {
      const cards = await generateAICards(subject.trim(), topics.split(',').map((t) => t.trim()).filter(Boolean), cardCount, 'medio', existingFronts);
    ...
    <div className="max-w-3xl mx-auto pb-24 animate-fade-in space-y-5">
      ...
      {mode === 'ia' ? (
        ...
        <section className="bg-[#0b1a2a]/90 border border-[#adc6ff]/20 rounded-3xl p-6 sm:p-8 shadow-2xl">
          ...
          <ManualCardForm existingDecks={decks.map((deck) => deck.title)} subjects={Array.from(new Set(decks.flatMap((deck) => [deck.category, ...deck.cards.map((card) => card.subject).filter((s): s is string => !!s)])))} onAddCardDirectly={handleManualCard} />
        </section>
      )}

      {generatedCards.length > 0 && mode === 'ia' && (
        <section className="space-y-3">
          <div className="flex items-center justify-between px-2"><h2 className="text-base font-extrabold text-[#adc6ff]">Cards gerados ({generatedCards.length})</h2><button type="button" onClick={() => setGeneratedCards([])} className="text-xs text-slate-500 hover:text-rose-400">Descartar</button></div>
          ...
          <button type="button" onClick={handleSaveGenerated} className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-extrabold flex items-center justify-center gap-2"><Save className="w-5 h-5" /> Salvar no Baralho</button>
        </section>
      )}
    </div>
  );
