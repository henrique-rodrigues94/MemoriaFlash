...
import { Check, ChevronDown, ChevronUp, GraduationCap, Loader2, Sparkles, Wand2, X, Lock } from 'lucide-react';
...
export const CurriculumPlannerView: React.FC<CurriculumPlannerViewProps> = ({ decks, stats, onSaveNewDeck, onOpenSubscription, onOpenAdvanced }) => {
  ...
  useEffect(() => {
    if (subject.trim().length < 2) {
      setDetectedLevels([]); setLevelCurricula(new Map()); setSelectedSubtopics(new Set<string>()); setExpandedTopics(new Set()); setBankAvailability(null); return;
    }
    let cancelled = false;
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (levels.length > 0) {
          await loadAllLevelCurricula(subject.trim(), levels, updated => { if (!cancelled) setLevelCurricula(new Map(updated)); });
        } else {
  ...
  useEffect(() => {
    ...
    setSelectedSubtopics(new Set<string>(flattenSubtopics(curriculum)));
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const result = await queryBankAvailability(subject.trim(), Array.from(selectedSubtopics) as string[], educationLevel);
      if (!cancelled) { setBankAvailability(result); setIsCheckingBank(false); }
    }, 350);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [subject, educationLevel, selectedSubtopics]);

  const toggleAll = () => setSelectedSubtopics(allSelected ? new Set<string>() : new Set<string>(allSubtopics));

  const toggleSubtopic = (subtopic: string) => {
    setSelectedSubtopics(current => { const next = new Set<string>(current); if (next.has(subtopic)) next.delete(subtopic); else next.add(subtopic); return next; });
  };
  ...
  const toggleExpanded = (category: string) => setExpandedTopics(current => { const next = new Set<string>(current); if (next.has(category)) next.delete(category); else next.add(category); return next; });

  const handleLevelChange = (level: EducationLevel) => {
    setEducationLevel(level); setEducationLevelLocked(true); setSelectedSubtopics(new Set<string>()); setBankAvailability(null);
  };

  const handleGenerate = async () => {
    if (!subject.trim()) { setError('Digite a matéria ou assunto.'); return; }
    if (selectedSubtopics.size === 0) { setError('Selecione pelo menos um tópico ou subtópico.'); return; }
    setIsGenerating(true); setError(null); setMessage(null);
    try {
      const missingTopics = bankAvailability?.needsGeneration || [];
      if (missingTopics.length > 0) await requestCurriculumPreparation({ subject: subject.trim(), educationLevel });
      if (readyNewEstimate <= 0) {
        setMessage(missingTopics.length > 0 ? 'A grade está selecionada. Os cards que ainda não existem no banco foram enviados para preparação pelo Content Agent. Você será beneficiado quando o conteúdo ficar disponível.' : 'Você já possui todos os cards disponíveis para os tópicos selecionados.');
        return;
      }
      const target = stats.isPro ? readyNewEstimate : Math.min(200, remainingToday, readyNewEstimate);
      if (target <= 0) { setError('Você já atingiu o limite diário de 200 cards. Amanhã poderá continuar de onde parou.'); return; }

      const selected: string[] = Array.from(selectedSubtopics) as string[];
      const existingFronts = new Set<string>();
      for (const deck of decks) for (const card of deck.cards || []) if (normalize(card.subject || deck.category || deck.title) === normalize(subject)) existingFronts.add(normalize(card.front));
      ...
      while (remainingToGenerate > 0 && safetyRounds < 20) {
        ...
        const existingFrontList: string[] = Array.from(existingFronts) as string[];
        const cards = await generateAICards(subject.trim(), selected, batchSize, educationLevel, existingFrontList);
        if (!cards.length) break;
        let newCount = 0;
        for (const card of cards) {
          const key = normalize(card.front);
          if (!key || existingFronts.has(key)) continue;
          existingFronts.add(key); generatedCards.push(card); newCount += 1;
        }
        if (newCount === 0) break;
      ...
      if (!generatedCards.length) { setMessage('Os cards selecionados já estão no seu baralho ou o banco ainda está preparando conteúdo novo.'); return; }
      const title = subject.trim();
      const existingDeck = decks.find(deck => normalize(deck.title) === normalize(title));
      const updatedDeck = existingDeck ? { ...existingDeck, cards: [...generatedCards, ...existingDeck.cards] } : makeDeck(title, generatedCards);
      onSaveNewDeck(updatedDeck);
      if (stats.isPro) setMessage(`✓ ${generatedCards.length} cards adicionados. O conteúdo faltante continua sendo preparado pelo Agent quando necessário.`);
      else if (generatedCards.length >= 200 || remainingToday <= generatedCards.length) setMessage(`✓ ${generatedCards.length} cards adicionados hoje. Amanhã você poderá continuar preenchendo esta mesma matéria.`);
      else setMessage(`✓ ${generatedCards.length} cards adicionados. Ainda há conteúdo disponível para continuar preenchendo a matéria.`);
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto pb-24 space-y-5 animate-fade-in">
      <div className="rounded-3xl bg-gradient-to-br from-[#242a52] to-[#161a35] text-white p-6 sm:p-8 shadow-xl border border-[#6d63ff]/20">
        <div className="flex items-start justify-between gap-4">
          ...
          {onOpenAdvanced && <button type="button" onClick={onOpenAdvanced} className="shrink-0 px-3 py-2 rounded-xl bg-white/10 hover:bg-white/15 border border-white/15 text-[11px] font-bold">Gerador avançado</button>}
        </div>
      </div>
      ...
      <section className="rounded-3xl bg-white border border-slate-200 p-5 sm:p-7 shadow-sm space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-2"><label className="text-xs font-black uppercase tracking-wider text-slate-600 flex items-center gap-1.5"><GraduationCap className="w-4 h-4" /> Nível de ensino</label>{isLoadingLevels && <Loader2 className="w-4 h-4 animate-spin text-[#6658f5]" />}</div>
          <div className="flex flex-wrap gap-2">
            {(detectedLevels.length ? detectedLevels : EDUCATION_LEVEL_META).map((level: any) => {
              const value = level.level || level.value; const label = level.label; const active = educationLevel === value;
        ...
        {curriculum.length > 0 && <div className="space-y-3">
          <div className="flex items-center justify-between gap-3 flex-wrap"><div><p className="text-xs font-black uppercase tracking-wider text-slate-700 flex items-center gap-1.5"><Wand2 className="w-4 h-4 text-[#6658f5]" /> Grade curricular</p><p className="text-[11px] text-slate-500 mt-1">{selectedSubtopics.size} de {allSubtopics.length} subtópicos selecionados</p></div><button type="button" onClick={toggleAll} className="px-3.5 py-2 rounded-xl bg-[#6658f5]/10 text-[#5143d9] border border-[#6658f5]/20 text-xs font-black flex items-center gap-1.5"><Check className="w-3.5 h-3.5" /> {allSelected ? 'Desmarcar todos' : 'Selecionar todos'}</button></div>
          <div className="rounded-2xl border border-slate-200 overflow-hidden divide-y divide-slate-100">
            {curriculum.map(category => {
              const subtopics = category.topics || []; const selectedCount = subtopics.filter(item => selectedSubtopics.has(item)).length; const topicSelected = selectedCount === subtopics.length && subtopics.length > 0; const partiallySelected = selectedCount > 0 && selectedCount < subtopics.length; const expanded = expandedTopics.has(category.category);
              return <div key={category.category} className="bg-white"><div className="flex items-center gap-3 px-4 py-3.5"><button type="button" onClick={() => toggleTopic(category)} className={`w-5 h-5 rounded-md border flex items-center justify-center shrink-0 ${topicSelected ? 'bg-[#6658f5] border-[#6658f5]' : partiallySelected ? 'bg-[#6658f5]/20 border-[#6658f5]' : 'border-slate-300'}`} aria-label={`Selecionar ${category.category}`}>{topicSelected && <Check className="w-3.5 h-3.5 text-white" />}{partiallySelected && <span className="w-2 h-0.5 rounded-full bg-[#6658f5]" />}</button><button type="button" onClick={() => toggleExpanded(category.category)} className="flex-1 text-left min-w-0"><span className="font-black text-sm text-slate-800">{category.category}</span><span className="block text-[11px] text-slate-500 mt-0.5">{selectedCount}/{subtopics.length} subtópicos</span></button><button type="button" onClick={() => toggleExpanded(category.category)} className="p-2 text-slate-400">{expanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</button></div>{expanded && <div className="px-4 pb-4 pl-12 grid sm:grid-cols-2 gap-2">{subtopics.map(subtopic => { const selected = selectedSubtopics.has(subtopic); return <button key={subtopic} type="button" onClick={() => toggleSubtopic(subtopic)} className={`text-left px-3 py-2.5 rounded-xl border text-xs flex items-center gap-2 transition ${selected ? 'bg-[#6658f5]/8 border-[#6658f5]/30 text-slate-800' : 'bg-slate-50 border-slate-200 text-slate-500'}`}><span className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${selected ? 'bg-[#6658f5] border-[#6658f5]' : 'border-slate-300 bg-white'}`}>{selected && <Check className="w-3 h-3 text-white" />}</span><span>{subtopic}</span></button>; })}</div>}</div>;
            })}
          </div>
        </div>}
        ...
        {bankAvailability && selectedSubtopics.size > 0 && <div className="rounded-2xl bg-slate-50 border border-slate-200 p-4 space-y-2"><div className="flex items-center justify-between gap-3"><span className="text-xs font-black text-slate-700">Cards disponíveis para sua seleção</span><span className="text-sm font-black text-[#5143d9]">{readyNewEstimate}</span></div><p className="text-[11px] text-slate-500">{bankAvailability.available.length} subtópicos já possuem conteúdo pronto e {bankAvailability.needsGeneration.length} ainda precisam ser preparados.</p>{!stats.isPro && <p className="text-[11px] text-[#5143d9] font-bold">Plano gratuito: até 200 cards por dia. Você pode voltar amanhã e continuar preenchendo esta mesma matéria.</p>}{stats.isPro && <p className="text-[11px] text-emerald-600 font-bold">PRO: sem limite diário de geração. O app entrega os cards disponíveis sem exigir seleção manual de quantidade.</p>}</div>}
