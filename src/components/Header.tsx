    ...
    <header className={`fixed top-0 left-0 w-full h-16 backdrop-blur-md z-40 border-b flex items-center justify-between px-3 sm:px-6 transition-colors duration-300 ${isLightTheme ? 'bg-[#EEF0F8]/92 border-[#CDD2E8] text-[#1A1F36] shadow-sm shadow-[#4F6EF7]/06' : 'bg-[#051424]/90 border-[#424754]/20 text-[#d4e4fa]'}`}>
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div onClick={() => setActiveTab('home')} className="cursor-pointer flex items-center gap-1.5 group">
          <span className={`font-bold text-lg sm:text-xl tracking-tight transition-colors ${isLightTheme ? 'text-[#4F6EF7] group-hover:text-[#3D57D9]' : 'text-[#adc6ff] group-hover:text-white'}`}>MemoriaFlash</span>
        </div>
        ...
        {activePro && (
          <button onClick={onOpenSubscription} className={`px-2.5 py-1 rounded-full border text-[11px] font-mono font-extrabold flex items-center gap-1.5 cursor-pointer hover:scale-105 transition-transform ${isLightTheme ? 'bg-amber-50 text-amber-700 border-amber-300' : 'bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-amber-300 border-amber-500/40'}`} title={stats.proPlanType === 'monthly' ? 'Sua conta é PRO — Plano Mensal' : stats.proPlanType === 'annual' ? 'Sua conta é PRO — Plano Anual' : stats.proPlanType === 'referral' ? 'Sua conta é PRO por indicação' : 'Sua conta é PRO'}>
            ...
            {stats.proPlanType === 'monthly' ? 'Plano Mensal' : stats.proPlanType === 'annual' ? 'Plano Anual' : stats.proPlanType === 'referral' ? 'PRO · Indicação' : 'PRO'}
          </button>
        )}
      </div>
        ...
        {!isGoogleLoggedIn && (
          <button onClick={onOpenAuth} className={`px-3 py-1.5 rounded-xl font-bold text-xs flex items-center gap-2 transition-all shadow-md cursor-pointer ${isLightTheme ? 'bg-white hover:bg-[#E5E8F4] text-[#1A1F36] border border-[#CDD2E8]' : 'bg-white hover:bg-slate-100 text-slate-900'}`} title="Entrar com sua Conta do Google">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24">
              ...
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06 0 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
            </svg>
