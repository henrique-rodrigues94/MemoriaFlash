...
function ensureStyles() {
  ...
  style.id = 'memoriaflash-feedback-v2-styles';
  style.textContent = `
    ...
    .dark .mf-feedback-v2-btn,.theme-dark .mf-feedback-v2-btn{background:rgba(15,23,42,.92);color:#fda4af;border-color:rgba(248,113,113,.35)}
    ...
    .theme-dark .mf-feedback-v2-modal{background:#0b1a2a;color:#fff;border-color:rgba(148,163,184,.2)}
    .mf-feedback-v2-title{font-size:17px;font-weight:900}.mf-feedback-v2-sub{margin-top:4px;margin-bottom:15px;font-size:11px;color:#66708d}.theme-dark .mf-feedback-v2-sub{color:#94a3b8}
    .mf-feedback-v2-reasons{display:grid;gap:8px}.mf-feedback-v2-reason{width:100%;padding:11px 12px;text-align:left;border-radius:12px;border:1px solid #d9deec;background:#f7f8fc;color:#303750;font-size:12px;font-weight:800;cursor:pointer}.theme-dark .mf-feedback-v2-reason{background:#122131;color:#cbd5e1;border-color:rgba(148,163,184,.18)}.mf-feedback-v2-reason.active{border-color:#6658f5;background:rgba(102,88,245,.12);color:#5143d9}
    .mf-feedback-v2-text{width:100%;min-height:80px;margin-top:12px;padding:11px;border-radius:12px;border:1px solid #d9deec;background:#fff;color:#1a1f36;outline:none;resize:vertical;font-size:12px}.theme-dark .mf-feedback-v2-text{background:#081522;color:#e2e8f0;border-color:rgba(148,163,184,.18)}
    .mf-feedback-v2-actions{display:flex;gap:8px;margin-top:12px}.mf-feedback-v2-actions button{flex:1;padding:11px;border-radius:12px;font-size:12px;font-weight:900;cursor:pointer}.mf-feedback-v2-cancel{border:1px solid #d9deec;background:#eef0f8;color:#303750}.theme-dark .mf-feedback-v2-cancel{background:#122131;color:#cbd5e1;border-color:rgba(148,163,184,.18)}.mf-feedback-v2-send{border:1px solid #6658f5;background:#6658f5;color:#fff}.mf-feedback-v2-send:disabled{opacity:.45}.mf-feedback-v2-error{margin-top:10px;color:#b4232d;font-size:11px}.mf-feedback-v2-success{text-align:center;padding:18px}.mf-feedback-v2-success strong{display:block;font-size:16px}.mf-feedback-v2-success span{display:block;margin-top:5px;color:#66708d;font-size:11px}
    @media(max-width:640px){.mf-feedback-v2-btn{right:78px;top:9px;padding:7px 8px;font-size:10px}.mf-feedback-v2-btn span{display:inline}}
  `;
...
function openModal() {
  ...
  backdrop.id = 'memoriaflash-feedback-v2-backdrop';
  backdrop.className = 'mf-feedback-v2-backdrop';
  const modal = document.createElement('div');
  modal.className = 'mf-feedback-v2-modal';
  ...
  reasons.className = 'mf-feedback-v2-reasons';
  let selected: StudyFeedbackReason | null = null;
  const send = document.createElement('button');
  send.className = 'mf-feedback-v2-send'; send.textContent = 'Enviar feedback'; send.disabled = true;
  for (const reason of REASONS) {
    const button = document.createElement('button');
    button.type = 'button'; button.className = 'mf-feedback-v2-reason'; button.textContent = reason.label;
  ...
  text.className = 'mf-feedback-v2-text'; text.maxLength = 500; text.placeholder = 'Explique o problema (opcional)...';
  ...
  send.onclick = async () => {
    ...
    try {
      ...
      if (!current) throw new Error('Não foi possível identificar o card atual.');
...
function installButton(container: HTMLElement) {
  ...
  const button = document.createElement('button');
  button.type = 'button'; button.className = 'mf-feedback-v2-btn'; button.innerHTML = '⚠️ <span>Relatar problema</span>';
  button.title = 'Relatar erro, resposta incorreta, explicação ruim ou conteúdo desatualizado';
