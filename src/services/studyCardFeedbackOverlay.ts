...
function ensureStyles() {
  ...
  style.textContent = `
    /* O botão é fixo na barra superior da sessão, nunca dentro do card.
       O overlay só é criado enquanto #flashcard-flip-container existe. */
    .mf-feedback-v2-btn{position:fixed;top:calc(env(safe-area-inset-top,0px) + 10px);right:92px;z-index:70;border:1px solid rgba(248,113,113,.35);background:rgba(255,255,255,.96);color:#b4232d;border-radius:12px;padding:8px 10px;font-size:11px;font-weight:900;display:flex;align-items:center;gap:6px;backdrop-filter:blur(8px);box-shadow:0 4px 16px rgba(15,23,42,.12);cursor:pointer;white-space:nowrap}
    html:not(.theme-light) .mf-feedback-v2-btn{background:rgba(15,23,42,.94);color:#fda4af;border-color:rgba(248,113,113,.35)}
    ...
    @media(max-width:640px){.mf-feedback-v2-btn{right:86px;top:calc(env(safe-area-inset-top,0px) + 9px);padding:7px 8px;font-size:10px}.mf-feedback-v2-btn span{display:inline}.mf-feedback-v2-btn{max-width:118px;overflow:hidden;text-overflow:ellipsis}}
  `;
...
export function installStudyCardFeedbackOverlayV2() {
  ...
  const scan = () => {
    ...
    if (container) {
      ...
      // A sessão terminou ou foi fechada. Não deixe nenhum controle de feedback
      // residual na tela final.
      lastContainer.querySelector('.mf-feedback-v2-btn')?.remove();
      lastContainer = null;
    }
  };
  scan();
