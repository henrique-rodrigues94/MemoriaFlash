...
export type StudyFeedbackReason =
  | 'wrong_answer'
  | 'bad_explanation'
  | 'confusing_question'
  | 'duplicate_content'
...
function findCardFromStudyDom(): { card: Flashcard; subject: string; deckId: string } | null {
  ...
  for (const deck of decks) {
    const card = deck.cards.find((candidate) => {
      ...
      const back = normalize(candidate.back);
      return front === normalizedQuestion || back === normalizedQuestion;
    });
    if (card) {
      return { card, subject: deck.category || deck.title, deckId: deck.id };
    }
  }

  // Fallback: ainda permite registrar o problema mesmo quando o estado local
  // demorou a sincronizar. O texto exibido fica armazenado como evidência.
  return {
    card: {
      id: `unknown-${Date.now()}`,
      front: question,
...
export function getCurrentStudyCard(): { card: Flashcard; subject: string; deckId: string } | null {
  return findCardFromStudyDom();
}

export async function submitStudyCardFeedback(payload: StudyFeedbackPayload): Promise<void> {
  const user = auth.currentUser;
  if (!user || user.isAnonymous) {
    throw new Error('Faça login para enviar um feedback sobre este card.');
  }
  ...
  await addDoc(collection(db, 'cardFeedback'), {
    ...
    comment: payload.comment?.trim().slice(0, 500) || null,
    userId: user.uid,
    deckId: deckId || null,
...
function ensureStyles(): void {
  ...
  style.textContent = `
    .mf-feedback-btn{position:absolute;right:12px;bottom:12px;z-index:20;border:1px solid rgba(248,113,113,.25);background:rgba(15,23,42,.82);color:#fda4af;border-radius:12px;padding:7px 10px;font-size:11px;font-weight:800;display:flex;align-items:center;gap:6px;backdrop-filter:blur(8px);cursor:pointer;transition:.2s}
    .mf-feedback-btn:hover{background:rgba(127,29,29,.5);border-color:rgba(248,113,113,.45);color:#fecdd3}
    .mf-feedback-backdrop{position:fixed;inset:0;z-index:100;display:flex;align-items:center;justify-content:center;padding:16px;background:rgba(2,6,23,.78);backdrop-filter:blur(8px)}
    .mf-feedback-modal{width:min(100%,420px);background:#0b1a2a;border:1px solid rgba(148,163,184,.2);border-radius:24px;padding:20px;box-shadow:0 25px 80px rgba(0,0,0,.5);color:#fff}
    .mf-feedback-title{font-size:16px;font-weight:900;margin-bottom:4px}.mf-feedback-sub{font-size:11px;color:#94a3b8;margin-bottom:16px}
    .mf-feedback-reasons{display:grid;gap:8px}.mf-feedback-reason{width:100%;padding:10px 12px;border:1px solid rgba(148,163,184,.18);background:#122131;color:#cbd5e1;border-radius:12px;text-align:left;font-size:12px;font-weight:700;cursor:pointer}.mf-feedback-reason.active{border-color:rgba(96,165,250,.65);background:rgba(37,99,235,.16);color:#bfdbfe}
    .mf-feedback-text{width:100%;min-height:78px;margin-top:12px;padding:10px 12px;border:1px solid rgba(148,163,184,.18);background:#081522;color:#e2e8f0;border-radius:12px;resize:vertical;outline:none;font-size:12px}.mf-feedback-text:focus{border-color:rgba(96,165,250,.6)}
    .mf-feedback-actions{display:flex;gap:8px;margin-top:12px}.mf-feedback-cancel,.mf-feedback-send{flex:1;padding:11px;border-radius:12px;font-size:12px;font-weight:900;cursor:pointer}.mf-feedback-cancel{background:#122131;border:1px solid rgba(148,163,184,.18);color:#cbd5e1}.mf-feedback-send{background:#2563eb;border:1px solid #3b82f6;color:#fff}.mf-feedback-send:disabled{opacity:.45;cursor:not-allowed}.mf-feedback-error{margin-top:10px;color:#fda4af;font-size:11px}.mf-feedback-success{text-align:center;padding:18px 4px}.mf-feedback-success strong{display:block;font-size:15px;margin-bottom:5px}.mf-feedback-success span{display:block;color:#94a3b8;font-size:11px}
  `;
...
function openFeedbackModal(): void {
  ...
  backdrop.id = 'memoriaflash-feedback-backdrop';
  backdrop.className = 'mf-feedback-backdrop';
  const modal = document.createElement('div');
  modal.className = 'mf-feedback-modal';
  ...
  reasons.className = 'mf-feedback-reasons';
  let selected: StudyFeedbackReason | null = null;
  for (const reason of FEEDBACK_REASONS) {
    const button = document.createElement('button');
    button.type = 'button'; button.className = 'mf-feedback-reason'; button.textContent = reason.label;
  ...
  text.className = 'mf-feedback-text'; text.maxLength = 500; text.placeholder = 'Explique o que está errado (opcional)...';
  ...
  send.onclick = async () => {
    ...
    try {
      ...
      send.disabled = false; cancel.disabled = false; send.textContent = 'Enviar feedback';
      error.textContent = err?.message || 'Não foi possível enviar o feedback.';
    }
  };

  actions.append(cancel, send);
  modal.append(reasons, text, actions, error);
...
function installButton(container: HTMLElement): void {
  ...
  const button = document.createElement('button');
  button.type = 'button';
  button.className = 'mf-feedback-btn';
  button.innerHTML = '⚠️ <span>Problema no card?</span>';
...
export function installStudyCardFeedbackOverlay(): void {
  ...
  const scan = () => {
    const container = document.getElementById('flashcard-flip-container');
    if (container) installButton(container);
  };
  scan();
