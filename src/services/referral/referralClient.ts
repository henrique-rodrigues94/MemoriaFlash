...
export function buildReferralLink(code: string): string {
  return `${getPublicReferralBaseUrl()}/?ref=${encodeURIComponent(code)}`;
}
...
export function capturePendingReferralFromURL(): void {
  ...
  if (ref && !localStorage.getItem(PENDING_REF_KEY)) localStorage.setItem(PENDING_REF_KEY, ref.toUpperCase().trim());
}
...
export async function claimReferralCode(referralCode: string): Promise<ClaimReferralResult> {
  ...
  try {
    ...
    if (res.ok && data.success) {
      return { success: true, message: data.message || 'Indicação registrada com sucesso.', rewardDays: data.rewardDays, alreadyRewarded: data.alreadyRewarded };
    }
