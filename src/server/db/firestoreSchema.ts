...
export function normalizeText(text:string):string{return(text||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^\w\s-]/g,'').replace(/\s+/g,' ').trim();}
...
export interface CardBucketDoc{subject:string;topic:string;level:EducationLevel;cardType:CardContentType;cards:BankCard[];cardCount?:number;updatedAt:string;ttlAt:number;providerUsed:string;version?:number;contentVersion?:string;}
export type FeedbackRating='positive'|'negative'|'problem';
...
export interface CardFeedbackDoc{cardId:string;bucketId?:string;subject?:string;topic?:string;subtopic?:string;level?:EducationLevel;cardType?:CardContentType;difficulty?:string;rating:FeedbackRating;reportType?:'card_problem';reason?:FeedbackReason;comment?:string;userId?:string;question?:string;answer?:string;explanation?:string;curiosity?:string;deckId?:string;deckTitle?:string;sessionId?:string;cardPosition?:number;sessionTotal?:number;sessionProgress?:number;sessionStartedAt?:string;page?:string;appVersion?:string;platform?:string;createdAt:string;}
